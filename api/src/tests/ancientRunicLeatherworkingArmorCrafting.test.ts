...
describe("AncientRunicLeatherworkingArmorCraftingEngine Leatherworking & Reinforcement", () => {
    ...
    it("rejects crafting when insufficient hides are provided", () => {
        ...
        // Dragonscale Greaves requires 2x DRAGON_SCALE_HIDE. Only provided 1
        ...
        expect(frame.currentDurability).toBe(100); // Durability not deducted
    });

    it("handles crafting failure roll consuming durability", () => {
        const frame = AncientRunicLeatherworkingArmorCraftingEngine.constructRack("lw_03", "PINE_TANNING_FRAME", 100000); // 85% success

        // Roll 0.95 (95 > 85% success) -> Craft failure
        ...
        expect(frame.currentDurability).toBe(85); // 100 - 15
    });
