...
describe("AncientRunicLeatherHorseCrupperBenchEngine Dock Retention Benches & Tail Rigs", () => {
    it("crafts Celestial Void Valkyrie Sovereign Crupper in Dock Sanctum achieving 100% retention and returns spliced pelts", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_01", "CELESTIAL_VOID_VALKYRIE_DOCK_SANCTUM");
        expect(bench.benchType).toBe("CELESTIAL_VOID_VALKYRIE_DOCK_SANCTUM");
        expect(bench.currentDurability).toBe(350);

        const initialLeathers = [
            "CELESTIAL_VOID_ASTRAL_CRUPPER_PELT",
            "CELESTIAL_VOID_ASTRAL_CRUPPER_PELT",
            "CELESTIAL_VOID_ASTRAL_CRUPPER_PELT"
        ] as any[];
        ...
        expect(craftRes.crupper?.dockRetentionPercent).toBe(100);
        expect(craftRes.crupper?.finalForwardSlipMitigationPercent).toBe(100); // 84 * 1.20 = 100.8 -> clamped to 100%
        expect(craftRes.crupper?.finalTailDockComfortBonusPercent).toBe(77); // 64 * 1.20 = 76.8 -> 77%
        expect(craftRes.crupper?.consumedLeatherCount).toBe(2);
        expect(craftRes.crupper?.consumedLeatherType).toBe("CELESTIAL_VOID_ASTRAL_CRUPPER_PELT");
        expect(craftRes.crupper?.remainingProvidedLeathers.length).toBe(1);
        expect(craftRes.remainingDurability).toBe(340); // 350 - 10
    });

    it("verifies mid-range retention roll and sub-100% quality scaling on Elm crupper bench", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_mid", "ELM_CRUPPER_BENCH");
        ...
        // finalForwardSlipMitigation = Math.round(24 * 0.944) = 23
        // finalDockComfortBonus = Math.round(14 * 0.944) = 13
        const craftRes = AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(
            bench,
            "NOVICE_HILL_DOCK_CRUPPER",
            ["TANNED_BUFFALO_DOCK_STRAP", "TANNED_BUFFALO_DOCK_STRAP"],
            0.1,
            0.5
        );

        expect(craftRes.success).toBe(true);
        expect(craftRes.crupper?.dockRetentionPercent).toBe(36);
        expect(craftRes.crupper?.finalForwardSlipMitigationPercent).toBe(23);
        expect(craftRes.crupper?.finalTailDockComfortBonusPercent).toBe(13);
    });

    it("handles bench becoming non-functional after successful craft when durability falls below threshold", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_wear", "ELM_CRUPPER_BENCH");
    ...
    it("rejects crafting when insufficient leather is provided and returns provided leathers", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_02", "ELM_CRUPPER_BENCH");

        const failRes = AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(
            bench,
            "WARMASTER_MITHRIL_BOSS_CRUPPER",
            ["TEMPERED_MITHRIL_FORKED_BOSS_SET"]
        );

        expect(failRes.success).toBe(false);
        expect(failRes.reason).toContain("Insufficient dock straps/forked bosses");
        expect(failRes.remainingProvidedLeathers.length).toBe(1);
        expect(bench.currentDurability).toBe(95);
    });

    it("handles dock strap misaligned failure roll consuming durability and leathers", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_03", "ELM_CRUPPER_BENCH"); // 87% success

        const fail = AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(
            bench,
            "NOVICE_HILL_DOCK_CRUPPER",
            ["TANNED_BUFFALO_DOCK_STRAP", "TANNED_BUFFALO_DOCK_STRAP", "TANNED_BUFFALO_DOCK_STRAP"],
            0.95
        );
    ...
    it("gates isFunctional in maintainBench based on DURABILITY_COST_PER_CRAFT threshold and returns clone", () => {
        const bench = AncientRunicLeatherHorseCrupperBenchEngine.constructBench("leather_04", "ELM_CRUPPER_BENCH");
    ...
    it("guards against null inputs and unsupported bench models", () => {
        ...
        expect(AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(invalidBench, "NOVICE_HILL_DOCK_CRUPPER", ["TANNED_BUFFALO_DOCK_STRAP", "TANNED_BUFFALO_DOCK_STRAP"]).success).toBe(false);
        expect(AncientRunicLeatherHorseCrupperBenchEngine.craftCrupper(null as any, "NOVICE_HILL_DOCK_CRUPPER", []).success).toBe(false);
