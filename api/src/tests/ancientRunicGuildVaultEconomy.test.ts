...
describe("AncientRunicGuildVaultEconomyEngine Guild Banks & Multi-Sig", () => {
    it("creates Citadel Vault, levies 15% member tax into treasury, and handles multi-sig 3-of-3 withdrawal", () => {
        ...
        const proposal = AncientRunicGuildVaultEconomyEngine.proposeWithdrawal(vault, "gm_01", "member_01", 100, 100000);
        expect(proposal.signatures.size).toBe(1);
        expect(proposal.isExecuted).toBe(false);

        // Officer 2 signs (total 2/3 signatures)
        const sign2 = AncientRunicGuildVaultEconomyEngine.signWithdrawalProposal(vault, proposal, "officer_02");
        expect(sign2.success).toBe(true);
        expect(sign2.totalSignatures).toBe(2);
        expect(sign2.isExecuted).toBe(false);
        expect(vault.currentGoldBalance).toBe(150); // Not executed yet

        // Officer 3 signs (total 3/3 signatures -> executed!)
        const sign3 = AncientRunicGuildVaultEconomyEngine.signWithdrawalProposal(vault, proposal, "officer_03");
        ...
        expect(vault.currentGoldBalance).toBe(50); // 150 - 100
    });

    it("deposits items into vault and enforces item slot limit", () => {
        const vault = AncientRunicGuildVaultEconomyEngine.createGuildVault("g_hall", "GUILD_HALL_VAULT", 10, 100000); // 20 slots
        vault.maxItemSlots = 2; // For test capacity limit
    ...
    it("rejects proposal creation when amount exceeds vault gold balance", () => {
        ...
        expect(() => AncientRunicGuildVaultEconomyEngine.proposeWithdrawal(vault, "p1", "p2", 500)).toThrow(
            "amount exceeds treasury balance"
        );
    });

    it("rejects signing an already executed proposal", () => {
        const vault = AncientRunicGuildVaultEconomyEngine.createGuildVault("g", "GUILD_HALL_VAULT", 10); // 1 signature needed
        vault.currentGoldBalance = 100;

        const prop = AncientRunicGuildVaultEconomyEngine.proposeWithdrawal(vault, "p1", "p2", 50);
        // p1 signs (1/1 for Hall Vault -> executes immediately)
        const s1 = AncientRunicGuildVaultEconomyEngine.signWithdrawalProposal(vault, prop, "p1");
        expect(s1.isExecuted).toBe(true);

        const s2 = AncientRunicGuildVaultEconomyEngine.signWithdrawalProposal(vault, prop, "p2");
